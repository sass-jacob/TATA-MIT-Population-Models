...
# prefactor in front of transition rate
K_12_prefactor = pybamm.Parameter("Transition Rate Prefactor")
# additive factor in exponential term
K_12_additive_factor = pybamm.Parameter("Transition Rate Additive Factor")
...
def transition_rate(V):
    """Transition rate between untransformed and transformed populations."""
    return K_12_prefactor * pybamm.exp(thermal_voltage / ((V - phi_phase_separation)**2 + K_12_additive_factor))
...
params = pybamm.ParameterValues({
    ...
    "Transition Rate Prefactor": 1e-1,
    "Transition Rate Additive Factor": 1e-1,
...
# by varying the transition rate, I suspect that we can get a better match of the simulated
# voltage profile to what is expected for a cell. this motivates the parameter sweep for the
